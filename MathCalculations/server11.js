...
var data = require('../Nhieu_Joey_Assignment2/public/product_data.js');
...
app.post("/process_form", function(request, response, next){
   ...
   //Validates data
   var valid_qty = true; //Check for valid input. 
   var total_purchases = false; 
   for (i = 0; i < products.length; i++) {
       aqty = request.body[`quantity${i}`];
       if(isNonNegIntString(aqty) == false){
           valid_qty &= false; //Invalid data 
       ...
       if (valid_qty == true && total_purchases == true) { 
...
app.post("/process_quantity_form", function (request, response) {
    ...
    if (typeof POST['submitPurchase'] != 'undefined') {
                var has_valid_quantities=true; // assuming that the variable will be true// 
                ...
                if (has_valid_quantities && has_quantities) {
                    ...
                    users_reg_data[stock_avl] = qty - users_reg_data[stock_avl];
                }  
